...
app.get('/result',  async(req, res) => {  
    ...
    const sta = await mysql.query('statistics');
    var j;
    const obj12 = [];
    ...
    const alumin = await mysql.query('customerlist_a');
    const obj = [];
for(step = 0; step <alumin.length; step++){
    obj[step] ={
    Date : alumin[step].date  ,
    Price: parseInt(alumin[step].price.replace('.00','').replace(',','')) }
}

const lead = await mysql.query('customerlist_l');
    const obj1 = [];
for(step = 0; step <lead.length; step++){
    obj1[step] ={
    Date : lead[step].date  ,
    Price: parseInt(lead[step].price.replace('.00','').replace(',','')) }
}

const zinc = await mysql.query('customerlist_z');
    const obj2 = [];
for(step = 0; step <zinc.length; step++){
    obj2[step] ={
    Date : zinc[step].date  ,
    Price: parseInt(zinc[step].price.replace('.00','').replace(',','')) }
}

const copper = await mysql.query('customerlist_c');
    const obj3 = [];
for(step = 0; step < copper.length; step++){
    obj3[step] ={
    Date :  copper[step].date  ,
    Price: parseInt( copper[step].price.replace('.00','').replace(',','')) }
}

const tin = await mysql.query('customerlist_t');
    const obj4 = [];
for(step = 0; step <tin.length; step++){
    obj4[step] ={
    Date : tin[step].date  ,
    Price: parseInt(tin[step].price.replace('.00','').replace(',','')) }
}

const nickel = await mysql.query('customerlist_n');
    const obj5 = [];
for(step = 0; step <nickel.length; step++){
    obj5[step] ={
    Date : nickel[step].date  ,
    Price: parseInt(nickel[step].price.replace('.00','').replace(',','')) }
}

    res.render('index', {data:obj10 , data1:obj12, Alu: obj , Lead:obj1 , Zinc:obj2, Copper:obj3, Tin:obj4 , Nickel:obj5});
...
{
    await mysql.query('dele_non1');
    await mysql.query('dele_non2');
    await mysql.query('dele_non3');
    await mysql.query('dele_non4');
    await mysql.query('dele_non5');
    await mysql.query('dele_non6');
    await mysql.query('dele_non7');
  // 페이지 보안설정
